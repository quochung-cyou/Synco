...
class CallDependencyExtractorAgent(BaseAgent):
    @property
    def name(self) -> str:
        return "call_dependency_extractor_agent"
    ...
    async def run(self, context: AgentContext) -> AgentResult:
        try:
            ...
            for file_path, content in context.files.items():
                dependencies = self._extract_call_dependencies(content)
                results[file_path] = dependencies
                processed += 1
            
            progress = int((processed / total_files * 100)) if total_files > 0 else 100
            logger.info(f"Extracted CALL dependencies from {processed}/{total_files} files")
            ...
            logger.error(f"Error in CallDependencyExtractorAgent: {e}")
            return AgentResult(
                agent_name=self.name,
    ...
    def _extract_call_dependencies(self, file_content: str) -> List[dict]:
        dependencies = []
        call_pattern = re.compile(r'CALL\s+[\'"]([A-Z0-9\-_]+)[\'"]', re.IGNORECASE)
        
        for idx, line in enumerate(file_content.split('\n')):
            match = call_pattern.search(line)
            if match:
                target = match.group(1).upper()
                if not target.endswith('.CBL'):
                    target += '.CBL'
                dependencies.append({
                    "target": target,
                    "type": "CALL",
