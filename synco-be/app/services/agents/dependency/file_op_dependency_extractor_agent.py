...
class FileOpDependencyExtractorAgent(BaseAgent):
    @property
    def name(self) -> str:
        return "file_op_dependency_extractor_agent"
    ...
    async def run(self, context: AgentContext) -> AgentResult:
        try:
            ...
            for file_path, content in context.files.items():
                dependencies = self._extract_file_op_dependencies(content)
                results[file_path] = dependencies
                processed += 1
            
            progress = int((processed / total_files * 100)) if total_files > 0 else 100
            logger.info(f"Extracted file operation dependencies from {processed}/{total_files} files")
            ...
            logger.error(f"Error in FileOpDependencyExtractorAgent: {e}")
            return AgentResult(
                agent_name=self.name,
    ...
    def _extract_file_op_dependencies(self, file_content: str) -> List[dict]:
        dependencies = []
        file_ops = ['OPEN', 'CLOSE', 'READ', 'WRITE', 'REWRITE', 'DELETE']
        
        for op in file_ops:
            pattern = re.compile(rf'{op}\s+(INPUT|OUTPUT|I-O|EXTEND)?\s*([A-Z0-9\-_]+)', re.IGNORECASE)
            for idx, line in enumerate(file_content.split('\n')):
                match = pattern.search(line)
                if match:
                    target = match.group(2).upper() if match.group(2) else match.group(1).upper()
                    dependencies.append({
                        "target": target,
                        "type": op,
                        "line": idx + 1,
